"""Classify transactions from THIRD-most-recent statement per bank (Month 3 validation)."""
import os, io
...
for bank, email_list in bank_emails.items():
    if len(email_list) < 3:
        print(f"  [{bank}] Fewer than 3 statements found, skipping Month 3")
        continue
    msg_id, payload = email_list[2]   # index 2 = third most recent
    ...
    for ref in attachments:
        ...
        print(f"  [{bank}] parsed {len(txns)} transactions from third statement")
...
if not all_txns:
    print("\nNo transactions found for Month 3.")
    exit()
